...
const postRouter = createTRPCRouter({
    createPost: protectedProcedure
        ...
        .mutation(async ({ ctx, input }) => {
            ...
            const userLiked = createdPost.likes.some((like) => like.userId === ctx.session.user.id);
            const userDisliked = createdPost.dislikes.some((dislike) => dislike.userId === ctx.session.user.id);
